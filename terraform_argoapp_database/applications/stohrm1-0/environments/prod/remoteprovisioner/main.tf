module "remoteprovisioner" {
  ...
  eks_cluster_arn = data.terraform_remote_state.appeks.outputs.cluster-arn
  eks_cluster_namespace = var.eks_cluster_namespace
  eks_applicationcluster_url = data.terraform_remote_state.appeks.outputs.eks_cluster_serverurl
...
data "terraform_remote_state" "appeks" {
  backend = "s3"
  config = {
    bucket = "ascent-terraform-statefile"
    key    = "stohrmv1-0/prod/terraform.tfstate"
    region = "ap-south-1"
  }
}
data "terraform_remote_state" "ec2" {
