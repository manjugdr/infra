...
push_nginxlogs_timezone() {
    ...
    "value": [
      {
        ...
        "persistentVolumeClaim": {
          "claimName": "powerpayv2-1nginxlog-ppnginxingresslogschart-logs-pvc"
        }
      }
    ]
  },
  {
    ...
    kubectl patch service ingress-nginx-controller -n kube-system --type='json' -p='[
  {
    "op": "add",
    "path": "/spec/ports/-",
    "value": {
      "name": "rabbitmqmgmtsslport",
      "port": 15671,
      "protocol": "TCP",
      "targetPort": 15671
    }
  },
  {
    ...
    kubectl patch deployment ingress-nginx-controller -n kube-system --type='json' -p='[{"op": "add", "path": "/spec/template/spec/containers/0/args/-", "value": "--tcp-services-configmap=kube-system/tcp-services"}]'
...
main() {
    ...
    GITHUB_USERNAME=${args[5]}
    GITHUB_TOKEN=${args[6]}
    ARGOCD_CLUSTER_NAME=${args[7]}
    ARGOCD_APPLICATION_HELMREPO=${args[8]}
    ARGOCD_APPLICATION_HELMVALUES_FILENAME=${args[9]}
    ARGOCD_APPLICATION_NAME=${args[10]}
    ARGOCD_APPSVCNAME_HELMPATH=${args[11]}
